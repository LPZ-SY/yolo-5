...
# ---------------------------------- 训练超参数配置  ------------------------------------------------------
# DATA_CONFIG_PATH = r'D:\yolo-fall_system\sysytem\qq_3045834499\yolo11-falling\ultralytics\cfg\datasets\A_my_data.yaml' # 数据集配置文件路径
# D:\yolo-fall_system\sysytem\lpz\yolo11-falling\ultralytics\cfg\datasets\A_my_data.yaml
DATA_CONFIG_PATH = r'D:\yolo-fall_system\sysytem\lpz\yolo11-falling\ultralytics\cfg\datasets\A_my_data.yaml'
EPOCHS = 100      # 模型训练的轮数
...
#
# # yolov11s模型训练：训练模型的数据为'A_my_data.yaml'，轮数为100，图片大小为640，设备为本地的GPU显卡，关闭多线程的加载，图像加载的批次大小为4，开启图片缓存
...
# time.sleep(10) # 睡眠10s，主要是用于服务器多次训练的过程中使用
#
# # # yolov5n模型训练：训练模型的数据为'A_my_data.yaml'，轮数为100，图片大小为640，设备为本地的GPU显卡，关闭多线程的加载，图像加载的批次大小为4，开启图片缓存
...
# results = model.train(data=DATA_CONFIG_PATH, project="./runs/yolo11n_src", epochs=EPOCHS, imgsz=IMAGE_SIZE, device=DEVICE, workers=WORKERS, batch=BATCH, cache=CACHE, amp=AMP)  # CPU 开始训练
# time.sleep(10) # 睡眠10s，主要是用于服务器多次训练的过程中使用
#
# # # --------------------------------------------------模型精度提升实验----------------------------------
# model = YOLO("yolo11n-cbam.yaml") # load a pretrained model (recommended for training)
# results = model.train(data=DATA_CONFIG_PATH, project="./runs/yolo11n-cbam", epochs=EPOCHS, imgsz=IMAGE_SIZE, device=DEVICE, workers=WORKERS, batch=BATCH, cache=CACHE, amp=AMP)  # CPU 开始训练
# time.sleep(10) # 睡眠10s，主要是用于服务器多次训练的过程中使用

# #---------------------------  模型轻量化实验 -------------------------------------------------------
model = YOLO("yolo11n-GhostConv.yaml") # load a pretrained model (recommended for training)
results = model.train(data=DATA_CONFIG_PATH, project="./runs/yolo11n-GhostConv", epochs=EPOCHS, imgsz=IMAGE_SIZE, device=DEVICE, workers=WORKERS, batch=BATCH, cache=CACHE, amp=AMP)  # CPU 开始训练
time.sleep(10) # 睡眠10s，主要是用于服务器多次训练的过程中使用
